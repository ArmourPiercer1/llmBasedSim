...
`GameState` 是模拟世界的核心状态，使用 LangGraph `TypedDict` 定义。遵循"图内跑 dict，边界用 Pydantic"原则：状态在节点间以普通 dict 流转，Pydantic 只用于 LLM 输出解析、配置校验和存档边界。`game_graph.py` 构建 LangGraph tick 管道（10 节点）：
...
characters_all_decide   ← 所有 NPC 并发生成行动意图
  ↓
tick_speed_resolve      ← 动态 tick 速度判定（0.5~30 分钟）
  ↓
...
#### Locked 属性规则注入（`world_rules.locked_attributes`）

`locked: true` 属性的值由引擎自动计算，LLM 无法修改。通过 `locked_attributes` 列表声明计算规则，支持四种规则类型：

```yaml
world_rules:
  locked_attributes:
    - type: timer               # 条件满足时累加时间，否则重置为 0
      timer_key: alert_timer
      condition: "danger > 0"
      thresholds: [10, 30]
      warning: "警报已持续{threshold}分钟。"
    - type: stage               # 优先级条件链 → 单向不可逆的阶段推进
      stage_key: phase
      ...
      rules:
        ...
        - condition: "progress >= 50"
          stage: two
        - stage: one            # fallback
    - type: snapshot            # 保存属性当前值供下一 tick 比较
      source_key: value
      snapshot_key: _prev_value
    - type: list_constraint     # 条件触发时向列表属性追加值
      list_key: flags
      condition: "score > 3"
      value: alert
```

Condition 是纯布尔表达式，支持数值/字符串/布尔比较、`+` `-` `*` `/` 算术、`abs()` 函数、`or`/`and` 逻辑组合。所有标识符自动解析为实体属性值。规则按声明顺序执行。
...
- 初始化 Agent（对话式 + 文件式）；
- LangGraph tick 管道（10 节点，含玩家输入处理、行动可行性判断、动态 tick 速度、自然属性变化和 locked 属性自动计算）；
...
- 通用角色属性系统（`attributes` + `attribute_update`）——init 文件可为玩家/NPC 定义任意数值属性，节点按行动、事件和自然变化更新（自然变化使用 `natural_delta_per_minute` 字段，按 tick 实际游戏分钟数缩放）；支持隐藏属性（`hidden: true`），隐藏属性不在默认 UI 中展示，但影响检定和 NPC 行为；支持锁定属性（`locked: true`），LLM 无法修改，由 `world_rules.locked_attributes` 声明规则、引擎自动计算；
...
- Rich CLI（双面板渲染）；
- WebUI（`python -m src.web.main`）——暗色蓝灰 HUD 三栏界面、小说化叙事流、可折叠状态面板、感官/行动 modal、斜杠菜单、显示设置 localStorage 持久化、局域网手机访问（`--lan`）、管道节点实时进度、开局文件自动发现（`public_start/` + `private_start/`）、自定义路径开局、存档读取/保存；叙事历史持久化（`narrative_history`，刷新页面后完整恢复）；
- 《荷鲁斯崛起》系列开局场景——耳语山（`whisperheads.yaml`）、谋杀星（`murder.yaml`）；
- 属性更新管道重构——`natural_attribute_delta` 确定性节点拆分，自然 delta 和 locked 属性计算在感官节点之前同步执行，同 tick 属性变化可通过 `attribute_deltas` 瞬态字段进入叙事文本；
- Locked 属性规则注入系统（`world_rules.locked_attributes`）——timer/stage/snapshot/list_constraint 四种规则类型，纯布尔条件表达式求值器（`_eval_locked_condition`），`apply_deterministic_attributes()` 作为规则解释器执行；
- 引擎代码清理——从 Prompt 模板和 Python 代码中移除所有场景特定内容，引擎保持纯通用；
- 启动文件编写指南（`InitFileGuide.md`）——覆盖单文件/文件组两种格式、世界规则注入、属性系统、locked 属性规则等全部功能；
...
4. **自动化测试覆盖已大幅提升，但集成测试仍缺失**
   - 纯函数测试已覆盖 251 个用例：JSON 解析、Prompt 模板加载与渲染、配置校验、数据模型默认值、规则预判（能力/物理/技能）、状态应用（玩家/NPC/物品/对话/情绪/检定）、属性更新（自然 delta、确定性 locked 属性计算）、世界规则注入、UI 渲染、WebUI 快照/感官数据、初始化文件、游戏时间和事件压缩；
...
3. **补齐集成测试** — 在现有 251 个纯函数测试基础上，增加 mock LLM 完整 tick 集成测试、Prompt 输出样例测试、save/load CLI 行为测试。
