...
- 通用角色属性系统（`attributes` + `attribute_update`）——init 文件可为玩家/NPC 定义任意数值属性，节点按行动、事件和自然变化更新（自然变化使用 `natural_delta_per_minute` 字段，按 tick 实际游戏分钟数缩放）；支持隐藏属性（`hidden: true`），隐藏属性不在默认 UI 中展示，但影响检定和 NPC 行为；
