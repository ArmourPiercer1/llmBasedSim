...
- `index.html`：三页导航壳（play / inspector / workbench）；零外部资源
  （无 CDN / 无字体 / 无 npm）；`<script src="/static/app.js">` 唯一
  引用（绝对路径 = v1 先例 web/index.html 同形；ERR-P10-16 更正：
  原相对引用 `app.js` 于 `GET /` 路由面 404，页面仅
  `/static/index.html` 可达——相对/绝对钉面与路由闭集矛盾）。
...
| ERR-P10-16 | 路由面与钉面矛盾（页面引用 404） | G10 人工面验收前置自动化（Playwright 端到端页面加载）发现（Leader 独立复现）：index.html 静态引用 = 相对形式（`href="styles.css"` / `src="app.js"`，§3.9 原钉），而 `GET /` 路由（§3.8 W4 面）把该页挂在 `/` 路径 → 相对解析落 `/styles.css` / `/app.js`，二者不在 WEB_ROUTES 9 行闭集内 → 404 信封（Playwright 请求日志实测：文档 200 + 两静态引用 404 → 页面 JS 永不加载，`GET /` 面在真实浏览器不可用）；唯一可达 URL = `/static/index.html`（相对解析落 `/static/*` 闭集内）——57 平铺 + 6 边界零覆盖（无端到端页面加载测试；G10-5 人工面挂起故未暴露）；v1 先例（web/index.html L7/L152）= 绝对引用 `/static/*`（SOT L111 先例行同目录形态） | 页面两源同改绝对形式（views.py 页头/页尾源（`GET /` 路由面）+ static/index.html（`/static/index.html` 面）：`href="/static/styles.css"` / `src="/static/app.js"`，v1 先例同形）→ `GET /` 与 `/static/index.html` 双面可达；§3.9 钉面同步（`<script src="/static/app.js">` 唯一引用 + 本 errata 注记）；路由闭集 / 测试钉值零变化（无测试钉引用串；face t3 K8 扫描对绝对路径串零命中已复测）；3205 恒等式零影响（页面源内容面非测试函数） | G10 人工面验收前置裁决定案（Leader byte-verify：Playwright 请求日志（文档 200 / app.js 404 / styles.css 404）+ 修复后端到端绿 + 全量 3205/0 复测 + v1 先例引用形式实测） |
