...
**模块间依赖（DAG，零环；→ = import 方向）**：`deployment → {profiles, prompts/diagnostic（RuntimeDiagnostic 发射面，§3.2 L205）}`；`router → {profiles, deployment, prompts/diagnostic}`；`structured → prompts/assembler`（wire 模型，运行期 import）；`policy → {router, adapter（模块级：WireMessage/InferenceRequest 运行时构造面 + InferenceBackend Protocol 注解）, staleness（模块级，§3.6 步骤 9）, structured, deployment（模块级：build_llm_policy 参数注解面）, prompts/registry, prompts/assembler, prompts/diagnostic（模块级，decide 运行时发射面）；critic = decide() 步骤 7 内函数级懒 import（enable_critic 面，critic 落 W6，DAG 原「TYPE_CHECKING 仅」与 staleness/critic/deployment 缺边 = 汇总面遗漏，ERR-P6-10）}`；`staleness → core 冻结面`；`critic → {prompts/assembler（wire 模型，模块级）, core 冻结面（context 经 TYPE_CHECKING）}`；`prompts/assembler → {prompts/registry, prompts/diagnostic, core 冻结面, content.schemas}`；`prompts/registry → {prompts/diagnostic, core 冻结面, content.schemas}`；`prompts/diagnostic → {content.schemas (DiagnosticSeverity)}`（DAG 叶，仅 pydantic + 该跨包只读 import）。**prompts 包零 llm import（registry/assembler/diagnostic 仅依赖 core + content.schemas + 包内兄弟模块）；llm 包对 prompts 的 import 发生在 structured（wire 模型）、critic（wire 模型）、policy（TemplateStore + TokenEstimator + RuntimeDiagnostic）与 deployment（RuntimeDiagnostic 发射面，§3.2 L205）；诊断发射模块（deployment / router / policy / registry / assembler）对 `prompts/diagnostic` 的 `RuntimeDiagnostic` 类型为只读 import（`P6_RUNTIME_DIAGNOSTIC_CODES` 封闭集零外部模块 import；闭集检查 = model_validator 内部检查，diagnostic.py L44-50；deployment = 构造期发射面（6 构造点），policy = decide 运行时发射面（透传组装诊断 + 构造 PARSE_* 族，3 构造点，§3.6 步骤 1/5/6/7），router / registry / assembler = 纯函数运行时发射面（5/7/1 构造点））——wire 模型归 L0 层所有（Leader-A6）；零环**。
...
2. `.venv/bin/python -m ruff check src/engine_v2/llm src/engine_v2/prompts tests/engine_v2 scripts`（line-length 100，target py312）；
...
ERR-P6-13（W6 R1 盲审轮处置 + 闭合）：W6（dev 13 文件：`critic.py` 112 行 4 导出 + `test_critic.py` 139 行 8 函数 + `test_p6_gate_scenario.py` 1041 行 20 函数（g6_01..g6_20 = §5.2 1:1）+ `test_p6_adversarial.py` 633 行 9 函数（AD-1..AD-9）+ 8 测试夹具（v2_project_llm 6 + v2_deployment 2）+ `llm_smoke.py` 162 行；Leader hunk 2：`pyproject.toml` +httpx 依赖（#24）+ `test_import_boundary.py` L818-1231 TestP6Boundary 6 方法纯追加块（#37），wave 提交 23d40fe，gate ③ 白名单 diff = 恰 37 预验）R1 轮 4 审查员 = 4/4 PASS（工作流传输面 R2/R3 摘要截断、盘上报告完整在位——Leader 按盘上 JSON 验收，传输失败不重发，ERR-P6-7 同型），findings 15（3 DOC + 12 INFO，0 BLOCK/SUPPLEMENT，0 执行失败；引文核验 119/70/63/44 全部落盘复核；R1 实证 17 分支内存 critique 探针 17/17、全套件独立复跑 2813/0）。处置：F-50 采纳（F-R2-1 + R4-F1 同目标：§3.12 方法 2 排除条款「实测 42 处命中（11 名 = 40，langchain = 2）」= W1 R3 时点测量随文照抄，与当前冻结字节面漂移——独立复核：冻结 pre-P6 内容（L1-817）= 43 token（11 名 = 41，较 W1 R3 的 40 多 1 = W3 B3 P6 例外路径串，ERR-P6-6）+ W6 P6 块白名单路径串 +41（含裸 llm 段，排除设计面），现 AST 字符串字面量面 = 84（11 名 = 82，langchain = 2，与 R2 独立计数一致）；规范面（方法 2 排除机制、27 文件域算术、探针集合与 `P4_LLM_PROVIDER_BLACKLIST` 等价、0 命中结果）全部字节复核绿、无变化 → L519 原位修正加波次限定，不占补充预算。F-51 采纳（R3-3：ERR-P6-11 F-49 锚「registry.py L94-112」尾行少 1——冻结 `validate_template_ref` 实际跨度 L94-113（成功返回 `return resolved, None` 在 L113），锚内行为描述全部位于引用区间内、非阻塞锚滑移 → 原位修正 L94-113，不占补充预算）。其余 12 INFO 全部为披露/自裁登记面（零行动）：F-R1-01 `critique_instruction` 签名 `Sequence[str]` 拓宽 SOT §3.8 tuple 钉面（全部钉死调用点传 tuple，行为等价）；F-R1-02 gate 主链直接调 `decide()` 而非 `run_policy_decide` facade（facade 唯一增量 = B-CON-5 actor 匹配守卫，g6_20 已机械验证反例）；F-R1-03 S2 微面（5 层/L0 非空/L4 空/token_estimate>0）由冻结 W1 `test_assembler` L245/L277-293 钉死 + e2e 五零诊断 assemble 隐式强制；F-R2-2 P6 块 41 个 llm 路径 token 属排除设计面（F-50 漂移源）；F-R2-3 gate 文件零直接 httpx（冻结 adapter 模块命名空间获取，L105/L141 属性访问模式）；R3-1 `deployment_alt.yaml` 注释「零 12 名」= 零 12 名取值口径（字段名 `provider`/`base_url` 为冻结 `DeploymentEntry` schema 强制面，用户域文件不在 27 文件扫描域，brief 措辞最低权威）；R3-2 AD-8 双跑范围 = decide/assemble 全链含坏 JSON（e2e 世界双跑归 g6_17，SOT D-P6-19 L723 + §5.2 #17 + ERR-P6-10(a) 交叉支持）；R4-F2 方法 5 按 ERR-P6-12 修正口径逐字落地（允许集 + 边界自豁免，random/datetime、socket 族仍扫边界文件）；R4-F3 gate 文件 httpx 属性访问面披露（P6 测试树唯一直接 `import httpx` = 冻结 `test_adapter.py`）；R4-F4 AD-4 钉面硬断言耦合冻结 `DeploymentEntry` 字段名 + 夹具字节（未来 schema 改名/夹具编辑会断钉，非 W6 缺陷）；R4-F5 dev 升级风险三项全部核验为非 W6 缺陷（`proposal_id` 无 nonce = 冻结 W4 `structured.py` L184-187 规格固有 + 冻结 P4 scheduler keying 后果，g6_11 已钉语义，P8 评估面；t06 单文件 httpx 例外 = 冻结 W3 `test_adapter.py:45`，ERR-P6-6 口径；AD-4 耦合同 R4-F4）；R4-F6 纯追加结构核验（L818-1231 连续尾块、零新增顶层 import、全部引用解析至冻结定义、白名单序与 SOT 表 #1-37 一致、冻结套件 20/20 绿）。全部修正为纯文档面（零代码 diff，不占补充预算）。闭合状态：套件 2813/0（基线 2807 + TestP6Boundary 6）；隔离 llm 113 / prompts 25；bare 113/25；ruff 净（llm/prompts/tests/scripts + 边界文件）；TestP6Boundary 6/6 绿（方法 5 = ERR-P6-12 修正口径）；K8 12 名（含 langchain）27 文件域 0/0（Leader 独立复扫双口径：正式口径 0；过度严格纯子串散文变体 = 41 子串误报、0 正式命中）。s2 风险移交 G6：(1) gate ⑥ smoke dry-run = 进程内无凭据路径（exit 3 确定性），有凭据路径 = 手动专用真调（D-P6-17，不入 gate 机械面）；(2) dev 升级风险三项均为非 W6 缺陷（R4-F5，见上）；(3) 未来波次 P6 族新文件须扩展 TestP6Boundary 块（方法 1/2 fail-loud，无静默覆盖洞）；(4) 边界文件 12 名 AST 负载随后续 Leader hunk 增长属预期（排除机制不变，F-50 口径持续以独立复扫为准）。

ERR-P6-14（G6 R1 门禁盲审轮处置 + 闭合）：G6 门禁盲审轮（G6-R1，全新一轮全新盲）4 审查员 = 4/4 PASS（工作流传输面摘要截断、盘上报告完整在位——Leader 按盘上 JSON 验收，ERR-P6-7/13 同型），findings 7（3 DOC + 4 INFO，0 BLOCK/SUPPLEMENT，0 执行失败；引文核验 85/104/42/68 全部落盘复核；gate ①-⑥ 六步由 4 名 reviewer 独立复跑全绿且互相对账一致；唯一授权 git 命令各执行恰好一次、输出 37 行各自独立双向集合对账）。处置：F-G6-01 采纳（R1 F1-R1-DOC-1：SOT L125 模块依赖 DAG 句与磁盘 import 面漂移——4 子项全部 Leader 字节实证（11 模块 AST import map + RuntimeDiagnostic 构造点计数）：(a) critic → prompts/assembler 模块级边（critic.py L40；TYPE_CHECKING 块仅覆盖 core.context_provider L42-43）未列入 per-module 边与 llm→prompts 汇总句；(b) 诊断模块集误列 adapter/structured/critic（`prompts/diagnostic` 实际 importer 恰 = deployment/router/policy/registry/assembler）；(c)「其余仅类型级」与运行时发射矛盾（deployment 6 / policy 3 / router 5 / registry 7 / assembler 1 构造点）；(d) `P6_RUNTIME_DIAGNOSTIC_CODES` 零外部模块 import（闭集检查 = diagnostic.py model_validator L44-50 内部检查）。另 Leader 独立发现同句 5 处 per-module 缺边（router/policy → prompts/diagnostic、critic → prompts/assembler、assembler/registry → prompts/diagnostic）——同句、同字节核验面、同 hunk 修正。原位修正 = L125 七子编辑（5 per-module 边补全 + 汇总句 +critic（wire 模型）/policy 括注 +RuntimeDiagnostic + 诊断句重写含 6/3/5/7/1 构造点计数与封闭集零外部 import 陈述）。F-G6-02 采纳（R2-D1：§3.13 gate 运行序步骤 2（L582）ruff 命令缺 scripts 路径（smoke = 白名单 #36 会被字面复跑漏检）→ 补 ` scripts`）。F-G6-03 采纳（R2-D2：L17 环境行「httpx（传递依赖，可用）」= W0 期陈旧快照（W6 后 = 直接依赖 ≥0.28，白名单 #24）→ 原位修正）。其余 4 INFO 全部登记面（零行动）：F1-R1-INFO-1 gate ③ 顺序语义（`git diff --name-only` 恒按路径字母序输出、§3.13 表 = 波次序；「精确等值」= 集合 + 计数等值，历次闭合均按此语义执行、字面顺序敏感读法结构不可行——封闭集语义不变）；R2-I1 S4 措辞精度（python-dotenv = v1 时代既有依赖（P5 基线在位，src/main.py:11 / src/web/app.py:17 消费），P6 依赖 delta = 恰 +httpx>=0.28（pyproject L16）——风险评估不受影响，为 gate 报告精度登记）；R3-I1 / R4（两名独立盲审同靶命中：uv.lock root-project 块未随 pyproject httpx 增加重同步（lock requires-dist 缺 httpx）；uv.lock 按设计在 37 白名单与 gate ③ 路径域（src tests pyproject.toml scripts）之外，venv 实装 httpx 0.28.1，审查点全部机械步绿 → 不影响裁决，登记为 open risk；后续 = 维护波次跑 `uv lock` 重同步（非闭合前置；操作注记：clean env `uv sync` 不会装上 httpx））。全部修正为纯文档面（零代码 diff，不占补充预算）。闭合状态：套件 2813/0（修正后复验）；ruff 净（llm/prompts/tests/scripts + 边界文件）；gate ①-⑥ 全绿（③ = 恰 37；⑥ = fake e2e 20/20 + 进程内 smoke dry-run exit 3）；TestP6Boundary 6/6；core 回归 1959/0 + P5 gate 20/20；G6 六准则 6/6 met（4 名 reviewer 独立双重证据）；HARD STOP S1-S5 未触发（逐条见 gate 报告）。s2 风险：uv.lock 漂移（新增，维护波次面）+ ERR-P6-13 (1)-(4) 移交（smoke 有凭据路径 = manual-only；dev 升级风险三项均非 W6 缺陷（含 proposal_id → P8）；TestP6Boundary 块 fail-loud 属性；边界文件 12 名 AST 负载增长预期）。G6 裁决 = PASS（Leader gate 报告；未使用 CONDITIONAL PASS）。
