...
|---|---|---|---|---|
| G5-1 | 零 Python 项目可以 load + validate； | `LAYOUT_OPTIONAL` 封闭路径模板（§3.3）：缺省可选目录/文件 = 合法空；无 `pyproject.toml` ∧ 无 `plugins/` = 合法完整项目；参照项目 fixture（§3.12 #30-34，内容镜像 `public_start/test_empty.yaml` 的 world/player 节 + 顶层标量值（max_ticks/ticks_per_game_minute/game_time/narrative_style，fixture #30 之「meta 值」，§3.12 L556））经 `load_project → build_ir → validate_project` 全链 0 error | D-P5-04/05/13 | #1-3、#16 |
...
| LLM 策略内容（Director 决策层内容） | G4 §6-3/§6-4 内容层义务（§7-4 = 认识论边界/K4 面），非 T01-T10 任务面 | OI-P5-3（§10）+ D-P5-DEV-5（§8.4） |
...
| K4 | Prompt 不能定义世界权限（Spec:295-303） | `PromptPolicy` 字段封闭集 = {id, scope, template_ref, variables}，**无** authority/permission 字段；`InferenceCapabilityProfile` 无 provider/model/endpoint/credential 字段 | P5-INV-4 | 断言 #19 双证（#19a 12 名扫描 + #19b 字段集内省）→ W6 门禁面：`test_validator.py`（K8 探针表 #19a + 字段集内省 #19b，§6.1 L745）与 `test_p5_gate_scenario.py`（§5.2 断言 #19，§6.1 L747） |
...
**导入纪律（全部 P5 模块，D-P5-15）**：允许 import = stdlib 白名单（`typing` `re` `enum` `pathlib` `json` `argparse` `logging` `sys` `importlib.metadata` `collections.abc` `__future__`）+ `pydantic` + `yaml` + `src.engine_v2.core.serialization`（仅 `dump_json`/`load_json`/`assert_json_clean`）。禁止：`asyncio` `datetime` `time` `random` 网络族（`socket`/`http`/`urllib`/`requests` 等）、`src.*`（v1，含 `src.game.*`/`src.config.*`/`src.agents.*`）、`langgraph`/`openai`/`langchain*` 族、`importlib.import_module`/`__import__`/`importlib.util.spec_from_file_location`（G5-3 机械面，仅 `importlib.metadata` 允许）。`importlib.metadata` 的使用面被限制在 `plugins/registry.py` 内且**只读 metadata，零 import**（D-P5-08）。机械核验 = TestP5Boundary（§3.11/§6.4）。
...
- **ENGINE_VERSION**（`Final[str]`，值 `"0.5.0"`）：模块级私有常量，不入 `__all__`（25 导出不含之）；单点权威，registry 重导出、module_graph 与 manifest 面消费同一常量（D-P5-08、ERR-P5-3 S-A）。

**错误族**：模型构造期形状违例 = pydantic `ValidationError`（由 `project_ir.build_ir` 捕获转 `LLMSIM_SCHEMA`；本模块不直接产诊断——`Diagnostic` 自身 code 校验 = code ∉ `DIAGNOSTIC_CODES` → 构造期 `ValidationError` 拒绝，与形状违例同族）。
...
**依据**：v1 world 段形状对照 = test_empty.yaml world 段（name/description/environment/locations/objects——v1 `world.objects` 映射至 v2 顶层 `items` 分节（ObjectSpec，§3.1），不入 WorldSpec）；v1 项目顶层键 `max_ticks` / `game_time` / `ticks_per_game_minute` / `narrative_style` 属 `ScenarioSpec`（§3.1 后文表），**不属** WorldSpec。
...
- **build_ir(raw: RawProject) -> IRBuildResult**：
  ...
  2. game.yaml 顶层键封闭集（8 键）= {`manifest`, `scenario`, `player`, `component_schemas`, `authority`, `gameplay_modes`, `capabilities`, `plugin_descriptors`}；逐节 `model_validate`（`extra="forbid"`）：pydantic `ValidationError` → `LLMSIM_SCHEMA`（path = 文件路径，refs = `[e['loc'] 点分串, e['type']]`，每条 error 一条诊断，按 pydantic errors() 产出序（即模型字段声明序；ERR-P5-13 钉死））；`extra` 键 → `LLMSIM_UNKNOWN_KEY`（path = 文件路径，refs = `[键名]`）。player 节 → PlayerSpec（单值，非 tuple；extra=forbid，未知键 → LLMSIM_UNKNOWN_KEY，同其他节）。
  3. 各节文件（world/*.yaml 顶层键必为 `world`；characters/items/rules/actions/prompts/scenarios/modules/*.yaml 顶层键必为同名复数键；plugins/*/plugin.yaml 由 plugins 面消费，build_ir 不解析）：同 2 的校验（顶层键违例：期望键缺失 → 1 条 `LLMSIM_SCHEMA`（refs=[键名, missing]）+ 多余键 → 每键 1 条 `LLMSIM_UNKNOWN_KEY`，两族可同时出、该文件即停（不再做内层校验），W2 实现口径；ERR-P5-13 钉死）；节文件缺 = 合法空（D-P5-05）。world 节为单值字段（0 文件 → ir.world = None；1 文件 → WorldSpec；≥2 文件 → sorted 首文件 + 每余文件一条 LLMSIM_SCHEMA，定义见 §3.1 WorldSpec）。
...
- **check_module_versions(graph) -> list[Diagnostic]**：先**解析复扫面**（L346「复扫并报」执行条款，ERR-P5-14 钉死）：节点 casefold 序 × requires/optional 声明序，每声明串经 parse_requirement，解析错误 → LLMSIM_MODULE_VERSION（path=node.id，refs=[entry]），同一 (path, refs) 去重首次保留；再**边面**：对每条 version 约束非空的边（边模型无 version_range 字段，约束 = source 节点对应 kind 声明串首个解析至本边 target 元素恢复，归一形，ERR-P5-14 钉死）：target ∉ nodes → 跳过（归 check_unsatisfied_requires 报，防双报）；∈ nodes：版本比较 = 点分数字逐位（短者补 0）；`>=V` 不满足或精确不等 → LLMSIM_MODULE_VERSION（path=source，refs=[f"{target}:{version_range}", f"have {node.version}"]，version_range = 归一形如 `>=2`）。**节点 engine_version 面**：对每个 `engine_version` 非空的节点（casefold 序）：按 D-P5-06 版本文法解析（空 = 任意；`>=V` 比较——version_range 生产者 parse_requirement 仅产 ""/`>=V`，exact 形在边面不可达，exact 为节点/manifest engine_version 字段专属）；与 `ENGINE_VERSION` 用与边面相同的点分数字逐位比较（短者补 0）（exact 形 = 元组相等，`>=` 形 = 不小于）；不满足 → `LLMSIM_ENGINE_VERSION`（path=node.id，refs=[声明值, `ENGINE_VERSION`]）。
...
- **tokenize_dsl(expression: str) -> list[DslToken]**：**正则逐字对齐 v1 `_TOKEN_RE`（condition_eval.py:25-30）**（`\s*` 前缀 + number `\d+(?:\.\d+)?` / string `"[^"]*"` / name `[A-Za-z_][A-Za-z0-9_\.]*|[一-鿿][一-鿿A-Za-z0-9_\.]*` / op `<=|>=|!=|[+\-*/<>=(),;:]`）；不可解析片段 → `DslEvalError`（对齐 v1 :47-48 行为；错误消息含片段前 20 字符，对齐 v1 :48）。
...
| InTestNode / NotInTestNode | in / not_in | left/right: DslNode | :147-168（右可为列表或字符串：str → `str(left) in right`；list/tuple/set → `left in set(right)`；否则（v1 :154-156 / :166-168 直接 raise；`_to_set` :353-359 仅被四个集合关键字 subset/superset/intersects/disjoint 调用 :184-185）） |
...
- **parse_dsl(expression: str, path_label: str) -> DslParseResult**：结构解析（tokenize → 文法树；**全部 if 分支结构解析**——比 v1 急进跳过更严，§8.4 D-P5-DEV-3 披露）。结构检查面 = v1 parser 的 parse 时检查全集：if 形状（:72-93）、比较/集合 op 存在性（:119-146 的 token 判定）、函数名/arity（:243-273）、outcome 关键字 ∈ {allowed,blocked,uncertain}（:285-290，lowercase 化）+ uncertain prob 范围（:296-299）、末尾多余 token（expect_end :303-305）。根产生式：parse_dsl 的根节点必须为 IfChainNode——其余 22 种类落于根位置 = parse 错误（LLMSIM_DSL_PARSE）；v1 锚 = parse_if :73-75 非 if 开头 raise「条件表达式必须以 if(...) 开始」（顶层入口 evaluate_condition 仅调 parse_if，:35-39）；66 例集 test_invalid_syntax_raises（test_condition_eval.py:81-84）钉死裸顶层表达式（如 `player.sanity < 30`）非法。括号分组非独立节点种类（23 种类闭集无分组形，断言 #12 口径不变）：v1 parse_primary（:222-234）对 `(` 以 _looks_like_condition（:307-330）深度 0 前瞻分发——条件形（深度 0 首个决定 token 为比较运算符 / and·or·not / 集合关键字 / `,` → True；先行 value token（非关键字 name/number/string）跳过——66 例集 test_compound_boolean（:160-164）含前导值形 `(a < 1 or a > 2)`）→ 布尔组（parse_or 后消费 `)`，值为 bool）；否则算术组（parse_add_sub 后消费 `)`）；一元负号 :223-224。66 例集 4 个转录用例用括号：test_parentheses_and_precedence（:52-55）/ test_not_operator（:153-157）/ test_compound_boolean（:160-164）/ test_boolean_with_set（:313-317）。**不做**变量解析/值求值（validate 期无上下文可跑 = G5-6 的机械前提）。path_label = 非空标签（调用方契约；空 label = 域外——Diagnostic.path min_length=1（§3.1 L167）使空 label 构造响亮 fail-fast（pydantic ValidationError），永不抛承诺覆盖域内全部结构错误，ERR-P5-15 钉死）。
...
**等价性契约（G5-5 机械面）**：对任意 (expression, context)：v1 `evaluate_condition`（condition_eval.py:35-39）成功且不含"未达分支含垃圾"形态（§8.4 D-P5-DEV-3）与「单分支无 trailing `if(cond, outcome)`」形（v1 仅 cond 真时合法，v2 parse 错误——§8.4 D-P5-DEV-6）与「uncertain 表达式概率」形（v1 接受表达式，v2 仅字面——§8.4 D-P5-DEV-7）与「裸 player/target（无点号）truthy 形」（v1 `_resolve_variable` :348-351 经 `context.get` 解析为 player/target dict 不抛错，v2 DslContext 独立字段不在 variables → `DslEvalError`——§8.4 D-P5-DEV-9）；后三形 66 例集均不含（tests/test_condition_eval.py 全扫描核验，0 命中）→ 无门禁影响 ⟺ v2 `parse_dsl` + `evaluate_condition` 同值（feasibility + probability 逐位相等，float 按 `==`）；v1 raise `ConditionEvalError` ⟺ v2 在 parse_dsl（结构错）或 evaluate_condition（语义错）raise/产诊断。66 例 characterization 集验证（§6.2，断言 #11）。
...
- **check_duplicate_ids(ir) -> list[Diagnostic]**：池内判重（LLMSIM_DUPLICATE_ID，path=池名，refs=[id, 首次出现索引, 重复出现索引]（池 tuple 内 0-based 位置索引、字符串化；tuple 序 = build_ir sorted 路径合并序，确定性））：池 = locations / items（ObjectSpec）/ characters / player 单值（对 location/object/character id 撞名 = 合法，v1 独立命名空间口径）；**全局唯一池** = rules / actions / modules / prompts / scenarios（含默认 scenario id）/ component_schemas / authority / gameplay_modes / capabilities / plugin_descriptors（path=池名）。
- **check_references(ir) -> list[Diagnostic]**（LLMSIM_UNRESOLVED_REF，path=holder id，refs=[ref_kind, ref_value]）：`connection` 值 → location id 池；`relationship` 键 → character 池 ∪ {player_id}；`inventory`/`starting_inventory` 元素 → object 池（含 items）。（ir.world = None → locations 池空，location 型引用（connection 值）跳过，不产 UNRESOLVED_REF——合法空语义）
...
**定位**：`llmsim validate` 入口（T09）。导入：`argparse` `sys` `loader` `project_ir` `validator` + 渲染用 stdlib。**stdout 纪律（D-P5-12）**：`--json` 模式 stdout = 且仅 = `render_json` 输出；human 模式 = `render_human` 行 + 1 行汇总；任何模式下**零其他 stdout**（唯一例外：usage 错路径按 `main` 规格写 stderr 并返回 2；`--help` 路径 stdout 输出帮助文本并返回 0——D-02 规格沉默面，实现取 argparse 标准面；除此之外 P5 实现零 stderr 输出）。
...
**定位**：本地项目插件 manifest（Spec §28.1，"必须显式 manifest" L1520-1521；禁隐式扫描 L1528）的解析。导入：`schemas`（Diagnostic/_ContractModel）+ `pydantic`（Field/ValidationError）+ 仅 stdlib（与 L120 全局白名单一致，ERR-P5-14 补齐）。
...
- **parse_plugin_manifest(path_label: str, raw: Any) -> PluginManifestParseResult**：非 dict → (None, [LLMSIM_SCHEMA path=path_label])；字段违例 → LLMSIM_SCHEMA（refs=[loc, type]）；entrypoint 文法错 → LLMSIM_PLUGIN_ENTRY_INVALID（path=path_label，refs=[entrypoint 原值])。path_label = 非空文件键（调用方契约：discover_local_plugins 输入即文件键；空 label 为域外，ERR-P5-14 钉死）。
...
- **EntryPointSpec**（frozen）：`module: str`（点分标识符）, `attribute: str`（标识符）；classmethod `from_string(s: str) -> tuple[EntryPointSpec | None, Diagnostic | None]`（`module:Attribute` 文法；非法 → (None, LLMSIM_PLUGIN_ENTRY_INVALID path=s)；s 空串 = 域外（调用方契约：registry 路在调用本函数前将空值短路为 ENTRY_INVALID path=EP.name refs=[EP.value]，ERR-P5-14 钉死）。与 manifest.py 的文法实现共享口径（两个模块各自持有纯函数实现，禁止互导以维持 plugins 包内 DAG 简洁——文法正则常量在各自模块内同值定义，测试同值核验）。
...
**定位**：双路发现（本地 manifest + entry-point）+ 注册校验（G5-2/G5-3）。导入：`schemas`（含 `RawProject` 类型） `project_ir` `plugins.manifest` `plugins.api` `importlib.metadata` `pydantic`（仅 `ValidationError`，EP 路构造违例捕获，ERR-P5-14 补齐）。**机械面**：本文件 + `content/loader.py` 是 AST 封闭模式扫描对象（断言 #6）：源码中不得出现 `import_module` / `__import__` / `spec_from_file_location` / `module_from_spec` 调用或 `entry.load()` 模式。
...
- **discover_entry_point_plugins(group: str = "llmsim.plugins") -> tuple[PluginRegistry, tuple[Diagnostic, ...]]**：`importlib.metadata.entry_points(group=group)`（**仅 metadata 枚举**——EP 对象只读 `.name`/`.value`/`.distribution`，**零 `.load()` 调用、零 import**，D-P5-08）；逐 EP：EP.value 空串 → LLMSIM_PLUGIN_ENTRY_INVALID（path=EP.name，refs=[EP.value]，ERR-P5-14 钉死）；`EntryPointSpec.from_string(EP.value)`；非法 → LLMSIM_PLUGIN_ENTRY_INVALID（path=EP.value，refs=[distribution 名]——「origin=distribution 名」= 实现口径简写，ERR-P5-14 钉死）；合法 → 构造 PluginManifest(id=EP.name, version=EP.distribution 版本或 "0.0.0", entrypoint=EP.value)：字段 pattern 违例（id 64 边界 / version 文法）→ 每条 error 一条 LLMSIM_SCHEMA（path=distribution 名，refs=[loc 点分串, type]）并跳过该 EP 不注册（W5 实现口径，ERR-P5-14 钉死）；无违例 → RegisteredPlugin(manifest=所构造 manifest, source=ENTRY_POINT, origin=distribution 名)。**EP 接线披露**：`discover_entry_point_plugins` 在 P5 仅由单测钉死（断言 #5，metadata-only 证明）；EP 发现的管线接线（运行时注册消费）归 P6+（validate 面：无网络、entry point 无磁盘读取授权，D-P5-08）；其缺席于 `validate_project` 管线是刻意的 scope 线，非遗漏。
- **validate_plugins(registry: PluginRegistry, ir: ProjectIR, raw: RawProject | None = None, engine_version: str = ENGINE_VERSION) -> list[Diagnostic]**：
  ...
  3. 每个 `ir.plugin_descriptors` 条目 id ∉ registry.plugins → LLMSIM_PLUGIN_ENTRY_UNRESOLVED（severity=warning，path=descriptor.id，refs=[descriptor.source 值]，ERR-P5-14 钉死）（D-P5-08：运行时或可由其他分布提供，不阻塞 validate）。
...
| 29 | `tests/engine_v2/core/test_import_boundary.py` | 修改（纯追加 P5 块 + T06 单一受控偏离（仅 test_p5_integration.py 网络/进程 IO 类别豁免，§6.5 要求，provider/v1 零容忍保持），§3.11/§6.5，ERR-P5-16） |
...
- **选择**：(c)。全 schema `extra="forbid"`，豁免 5 处：`ObjectSpec.properties`、`PlayerSpec.capabilities`、`CharacterSpec.personality`、`LocationSpec.properties`、`PlayerSpec.physical_profile`（v1 自由 dict 语义面，`condition_eval.py:343-351` 变量解析依赖任意键；`GameplayModeSpec.params` 与 `RawProject.files` 同为 §3.1 字面开放 dict（值为自由 Any），但不属于该 v1 语义豁免清单）。
...
- **选择**：(b)。Kahn 堆式实现，heap 键 = `id.casefold()`；输出唯一确定序。环 = 空列表 + 每个 SCC 恰好 1 条 `LLMSIM_MODULE_CYCLE`（SCC 节点 casefold 排序入 refs）。版本文法 `\d+(\.\d+)*`，点数字串 padding 比较（点分数字逐位、短者补 0：`2 < 2.1`，补 0 后 `2.1 = 2.1.0`，ERR-P5-14 钉死；对齐 v1 `standard.attributes >= 2` 习惯，Spec §41:1970-1987；Spec §41 示例目标 id 为点分形态（standard.attributes），故模块 id pattern 独立于 §3.1 实体基础 pattern（点分允许）；实体/规则/动作等 id 不受影响）。
...
- **选择**：(b)。
- **理由与一致性**：K7 可复现面；v1 双非确定根源（`condition_eval.py:4` `import random as _random`、`state_apply.py:101` roll `random.random()`）在 v2 全部改为注入——roll 消费的 0.5 缺省留在 P6 runtime（§7.5 移交注），DSL 层只产概率不掷骰。`pyproject.toml` dependencies 零增删（D-P5-15 冻结面）。
...
| 20 | round-trip：对 zero_python 与 plugin fixture 的 IR，令 ir2 = ProjectIR.model_validate(yaml.safe_load(canonical_yaml(ir)))（数据级再验证，不经目录布局）；断言 ir2 == ir 且 canonical_yaml(ir2) == canonical_yaml(ir)（双 dump 字节稳定） | K7 |
...
| 测试文件 | 被测模块 | 用例族（最少覆盖） |
|---|---|---|
| `content/test_schemas.py` | §3.1 | 25 导出逐一构造；`DIAGNOSTIC_CODES` 18 枚闭集核验（集合相等）；`Diagnostic` 非法 code 构造期拒绝；每模型 `extra="forbid"` 未知键 → `ValidationError`（每模型 ≥1 例）；5 个开放 dict 豁免各 ≥1 例；`ProjectManifest` 字段闭集；`ProjectIR` 16 字段默认值面 ——用例面由恰好 10 个扁平测试函数实现（frozen 与 extra=forbid 两例以循环覆盖全部 24 模型，达成“每模型 ≥1 例”）|
...
| `content/test_module_graph.py` | §3.4 | `parse_requirement` 文法全族（含非法）；钻石图拓扑精确序列（断言 #10 同源数据）；3 环 → `topological_order` = `[]` + `find_cycles` 恰好 1 SCC `[a,b,c]`（`LLMSIM_MODULE_CYCLE` 诊断生产面 = W6 `validate_project`（§3.6：每环一条，path=min(node)，refs=节点序，ERR-P5-14 钉死），本文件只断言 SCC 列表；#9）；requires 缺失 → `LLMSIM_MODULE_REQUIRES_MISSING`（#18）；版本比较矩阵（`2`/`2.1`/`2.1.0`/`>=2`）；节点 `engine_version` 面（`>=` 不满足 / exact 不匹配 → `LLMSIM_ENGINE_VERSION`）；`conflicts` 双向命中 1 条 |
...
**rand 族处置（4 例）**：v1 期望值由 `_random`（condition_eval.py:4）非确定产生，原断言为恒真条件上的可行性点断言（3× `assert outcome.feasibility == "allowed"` + 1× `"uncertain"`，tests/test_condition_eval.py:342-367）。v2 转录时注入种子化 `DslRng`（conftest `seeded_rng`）：点断言保留 + 同种子精确值断言新增（v2 确定性是 v1 的**严格增强**，不弱化原断言——等价方向单向：v1 通过 ⟹ v2 通过）。
...
|---|---|---|---|---|
| A1 | duplicated ID | `check_duplicate_ids` | 每重复 id 恰好 1 条 `LLMSIM_DUPLICATE_ID`（path=池名，refs=[id, 首次出现索引, 重复出现索引]（池 tuple 内 0-based 位置索引、字符串化；tuple 序 = build_ir sorted 路径合并序，确定性）） | test_p5_adversarial + fixture #39 |
...
| K3 Authority-Commit 分离 | `AuthorityPolicy` 仅声明面，无执行字段 | §3.1 字段表（声明面）+ test_schemas extra=forbid 逐模型断言（行为面） |
...
| D-P5-DEV-9 | 裸 player/target（无点号）truthy 形：v1 `_resolve_variable`（condition_eval.py:348-351）对裸 player/target 经 `context.get` 解析为 context 中 player/target dict（非空 dict truthy、空 dict falsy），不抛错；v2 DslContext（L394）player/target 为独立字段，resolve_variable 其他分支（L398）仅查 variables → `DslEvalError` | v2 DslContext 三字段 typed context（D-P5-09，§3.5 L394）；18 码闭集无对应 warning 码、无诊断通道 | 66 例集不含（tests/test_condition_eval.py 全扫描核验，0 命中）→ 无门禁影响；§3.5 L398 + L417 已披露 |
| D-P5-DEV-10 | 空串 match 形（RuleSpec.match == `""`）：v1 加载期丢弃规则（deterministic_rules.py:126-128 空 pattern → warn+None；:89-91 match_action 非 None 而 pattern None → return None），v2 步 2「r.match 非空时先 re.compile」将 `""` 视为无过滤器 → 规则恒参与求值（无诊断） | v2 RuleSpec.match 可选（D-P5-05）；18 码闭集无对应 warning 码、无诊断通道（与 DEV-8 同构） | 66 例集不含（tests/test_rules.py 全扫描核验，0 命中）→ 无门禁影响；§3.5 步 2 已披露 |
...
**ERR-P5-11**（2026-08-31）：症状 → 设计盲审 R10（4/4 通过，0 BLOCK / 0 SUPPLEMENT / 10 DOC / 4 INFO，0 执行失败）达标，10 条 DOC 闭合：(1) L374 InTest/NotInTest 行 v1 依据列 `_to_set` 引用范围 :353-360 尾差 1 行（L360 为空白行，非方法组成；实含装饰器 :353-359）；(2) L366 tokenize_dsl「对齐 v1 :45-48 行为」——v1 不可解析分支实起于 :47（`if not match:`，raise :48），L45-46 为 while 循环头与 `_TOKEN_RE.match` 调用；(3) L238 WorldSpec 依据行漏 v1 world 段 objects 键（test_empty.yaml L29-88 四 object：oak_door/light_crystal/old_parchments/wooden_crates；本文 L55 基线已列 world{…,objects}）且未说明其 v2 去向；(4) L663 D-P5-15 末句「dependencies 零增删（D-P5-01 冻结面）」交叉引用编号错误——D-P5-01（L575）为模块落位/core 零触碰决策，其文本无 dependencies 条款；dependencies 冻结在文档他处均归 D-P5-15（§5.3 #3、§3.11 pyproject 行、白名单 #11）；(5) L390 parse_dsl 括注「条件形（比较/and/or/not/集合关键字开头）」对 v1 `_looks_like_condition`（:307-330）不精确——v1 深度 0 前瞻跳过先行 value token（非关键字 name/number/string），以首个决定 token 分派（比较运算符 / and·or·not / 集合关键字 / `,` → True），66 例集恰含该前导值形（test_compound_boolean :160-164 `(a < 1 or a > 2)`），parity 门禁可捕获；(6) L36 G5-1 行「world/player/meta 结构」——test_empty.yaml 顶层 8 键（L1/89/135/136/147/148/151/152 逐一核验）无名为 meta 之节，「meta 值」= L556（fixture #30 规格）闭合之 4 个顶层标量值简称；(7) L417 等价性契约「对任意 (expression, context)」量化过宽——裸 player/target（无点号）truthy 形 v1 成功（`_resolve_variable` :348-351 经 `context.get` 解析为 player dict，非空 truthy/空 falsy，不抛错），v2 DslContext player/target 独立字段（不在 variables），L398 其他分支钉死 → `DslEvalError`；该形不在 DEV-3/6/7 排除清单，66 例集不含（全扫描 0 命中）；(8) L366 正则括注漏 v1 `_TOKEN_RE` 之 `\s*` 前缀（v1 :25）；(9) L761 rand 族处置「原断言为区间/形状」不实——v1 四例实为恒真条件上的可行性点断言（3× `assert outcome.feasibility == "allowed"`：rand()<1.0 / rand(0,100)>=0 / randint(1,6)>=1 + 1× `"uncertain"`：if(rand(), uncertain:0.5; allowed)，tests/test_condition_eval.py:342-367；v1 :247/:253/:259 值域保恒真）；(10) L74「G4 §7-4 内容层义务」误引——内容层义务 = G4 §6-3（L207-209 策略内容重提案）/§6-4（L210-212 BehaviorPolicy/ModePolicy 两协议）；G4 §7-4（L235-237）= 认识论边界不变量/K4 面。另 2 条 v1/v2 偏差登记（Leader 裁定）：D-P5-DEV-9 = 裸 player/target truthy 形（源自 (7)，R10-4 4-D1 审阅者建议）；D-P5-DEV-10 = 空串 match 形（源自 R10-4 4-I1 INFO，按审阅者建议依 DEV-8 体例升格：match 存在但为 `""` 时 v1 加载期丢弃规则（deterministic_rules.py:126-128 + :89-91），v2 步 2 将 `""` 视为无过滤器、规则恒参与求值无诊断；66 例集不含，无门禁影响）。→ 裁定与修正（Leader 2026-08-31 裁定；fixer K 应用）：DOC-1 `:353-360` → `:353-359`；DOC-2「对齐 v1 :45-48 行为」→「对齐 v1 :47-48 行为」；DOC-3 L238 依据行补 objects 键及 v2 去向（顶层 `items` 分节 ObjectSpec §3.1，不入 WorldSpec）；DOC-4 L663「D-P5-01 冻结面」→「D-P5-15 冻结面」；DOC-5 L390 括注改「深度 0 首个决定 token 分派 + 先行 value token 跳过 + 66 例集例」；DOC-6 L36「world/player/meta 结构」→「world/player 节 + 顶层标量值（max_ticks/ticks_per_game_minute/game_time/narrative_style，fixture #30 之「meta 值」，§3.12 L556）」；DOC-7 L417 排除清单第 4 项 = 裸 player/target truthy 形（→ D-P5-DEV-9），「后两形 66 例集均不含」→「后三形 66 例集均不含（…全扫描核验，0 命中）」；DOC-8 L366 括注补 `\s*` 前缀；DOC-9 L761「原断言为区间/形状」→「原断言为恒真条件上的可行性点断言（3× "allowed" + 1× "uncertain"，:342-367）」，「区间断言保留」→「点断言保留」；DOC-10 L74「G4 §7-4 内容层义务」→「G4 §6-3/§6-4 内容层义务（§7-4 = 认识论边界/K4 面）」；§8.4 新增 D-P5-DEV-9/DEV-10 两行，§8.3 交叉行偏差 8 → 10（D-P5-DEV-1..10）。INFO 3 条跳过（R10-2）：2-I1 re.error skip 运行时 logging 面未钉死（v1 warning 入加载期返回值列表而非运行时 log；§6.2 已钉「无诊断、无 caplog 断言」，66 例集不可观测，门禁无影响）；2-I2 NegNode 行锚 L384 :222-224 含 def 行（:222），L390 同处锚 :223-224——锚粒度不一，无语义影响；2-I3 P5_TEST_FILES 15 计数需跨引 P4_TEST_FILES 先例（test_import_boundary.py:209-220，口径排除边界文件本身；§6.4 公式印证 #29 不入 P5_TEST_FILES）——可解，不影响可照实现性。→ 影响面：**偏差 8 → 10（D-P5-DEV-9/10 登记，§8.3/§8.4 更新）；10 模块 / 20 断言 / 116 导出（25+6+6+11+43+8+4+3+3+7）/ 39 白名单 / 18 码 / 23 节点种类 / 66 parity / 13 对抗项 / 17 决策全部不变**；无新文件 / 新导出 / 新码 / 新决策；文档行数 989 → 992（+2 = §8.4 两新行、+1 = 本勘误行）；ERR-P5-1..10 文本未动。R10 报告存档于 `.review-drafts/p5-design-round10-reviewer-1..4.json`。本勘误后设计文档冻结（DESIGN FROZEN @ 本提交；P5 实现波次 W1-W6 按 §3.12 执行；G5 门禁盲审复核最终文档状态，如再生新勘误沿 append-only 链延续）。
**ERR-P5-12**（2026-08-31）：症状 → P5 W1 代码盲审 R1（4/4 通过，0 BLOCK / 0 SUPPLEMENT，0 执行失败；报告口径说明：盘存报告基线 6 DOC / 6 INFO（R1 2DOC+1INFO、R2 2DOC+1INFO、R3 0DOC+2INFO、R4 2DOC+2INFO），workflow 返回基线 8 DOC / 5 INFO——R3 盘存报告（0 DOC + 2 INFO，其 2 INFO 为 _ContractModel 共享基类行为等价、测试侧私有名 import）与 R3 workflow 返回值（2 DOC + 1 INFO）不一致，Leader 裁定以盘存报告为审查交付基线，R3 返回值 2 条 DOC 由 Leader SOT 核验后列入跳过清单）达标；8 条 DOC（盘存基线 6 + R3 返回值 2）裁定 6 采纳 / 2 跳过：(1) §2 L93 K4 行验证列 `test_schemas.py` 字段集内省断言（#19b）+ 12 名扫描（#19a）置错模块——§6.1 L745 将 #19a K8 探针表与 #19b 字段集内省均置于 test_validator.py 行，L747 将 §5.2 断言 #19 置于 test_p5_gate_scenario.py（test_assert_01..20），test_schemas.py 无字段集内省测试；(2) §8.1 L882 K3 行“test_schemas 字段集断言”——test_schemas.py 无 AuthorityPolicy 字段集断言，§6.1 L739 亦未要求，K3 机械面实为 §3.1 字段表（声明面）+ extra=forbid 逐模型断言（行为面）；(3) §6.1 L739 用例面（“25 导出逐一构造；…（每模型 ≥1 例）”）与实现的“恰好 10 个扁平测试函数”（其中 frozen 与 extra=forbid 两例循环覆盖全部 24 模型）无显式对应注记；(4) L131 ENGINE_VERSION 消费面“registry 重导出、module_graph import”漏 manifest 面（与 D-P5-08 L620“module_graph 与 manifest 面消费同一常量”口径不一）；(5) L133“除 `Diagnostic` 自身 code 校验”双读（构造期拒绝 vs 产诊断）——§6.1 L739“`Diagnostic` 非法 code 构造期拒绝”+ 实现（test_schemas.py L255 非法 code 构造期拒绝）钉死构造期拒绝读法；(6) D-P5-05 L602“豁免 5 处”清单与 §3.1 字面——`GameplayModeSpec.params`（L389）与 `RawProject.files`（L474）同为字面开放 dict（值为自由 Any），但不属于 v1 语义豁免清单，未注明易误读为“全 schema 仅 5 个开放 dict”。跳过（2 条，均为 R3 返回值 DOC）：(a) R3 返回值 DOC-1（K8 行 §2 L95 / §8.1 L886 引文“test_schemas import 面扫描”）——Leader SOT 核验：§2 K8 行实为 L97（验证列 = 断言 #19 api_key 探针描述，与 §6.1 L747 一致）、§8.1 K8 行实为 L887（12 名扫描 + schema 字段双证（#19），测试对象 = test_validator / test_cli，全 W6 面），引文不成立（审查者事实误引，与 R6-3 / R8-2 误引同族）；(b) R3 返回值 DOC-2（版本文法“3 处重复”）——引文前提不成立（所引方括号锚定 [0-9] 字符类模式全文 0 处，grep -cF 实测）；实际版本文法字面（D-P5-06 形式，见 L608 逐字）出现于 L276 / L345 / L471 / L608 共 4 处且逐字一致，L190 已以 D-P5-06 指针锚定——一致转录，INFO 级不改。4 条盘存 INFO（R1-INFO-1 L133 双读、R2-INFO-1“恰好 5 处”措辞、R4-INFO-1 L133 族、R4-INFO-2 开放 dict 穷尽性）分别被 D5 / D6 覆盖；R3 盘存 2 INFO 不处置。→ 裁定与处置（Leader 2026-08-31 裁定；Fixer L 执行）：D1–D6 六处 in-place 逐字修正（对应症状 (1)–(6)）；本勘误行 append（append-only 链延续 ERR-P5-11）。W1 代码交付物（schemas.py 548 行 / test_schemas.py 624 行 / 套件 2409 passed）不被本勘误修改（纯文档精度闭合）。→ 影响面：10 模块 / 20 断言 / 116 导出（25+6+6+11+43+8+4+3+3+7）/ 39 白名单 / 18 码 / 23 节点种类 / 66 parity / 13 对抗项 / 10 偏差 / 17 决策**全部不变**；无新文件 / 新导出 / 新码 / 新偏差 / 新决策；文档行数 992 → 993（+1 = 本勘误行）；ERR-P5-1..11 文本未动。W1 审查闭合（R1 一轮达标）→ 进入 W2（project_ir + loader + content 测试包 conftest/__init__，T02b/T03）。
**ERR-P5-13**（2026-08-31）：症状 → P5 W2 代码盲审 R1（4/4 通过，0 BLOCK / 0 SUPPLEMENT / 6 DOC / 10 INFO，0 执行失败；盘存报告与 workflow 返回值一致，套件 2434/0）达标；6 条 DOC 裁定（5 处文档修正 + 1 处代码侧测试修正，0 跳过）：(1) §3.2 L303“每条 error 一条诊断，按 loc 序”——“loc 序”无文法定义（未区分 loc 元组字典序与 pydantic 产出序）；W2 实现按 pydantic err.errors() 产出序（= 模型字段声明序）追加且测试 docstring 钉该读法；严格元组字典序读法排序不同（R1 实测：声明序 player_id → name → inventory.0/1，字典序则 inventory.* 应居先）；(2) §5.2 L719 断言 #20 公式 `canonical_yaml(ir_to_data(ir))` 与 §3.2 L309 签名 `canonical_yaml(ir: ProjectIR) -> str` 不一致（dict 照传必 AttributeError，R3 实测复现；R2/R3 双报告同根）；(3) §3.1 L120 stdlib 白名单封闭枚举缺 `__future__`——`from __future__ import annotations` 为全仓惯例（core 33 模块中 32 个 + W1 schemas.py + W2 两模块），W6 TestP5Boundary 若逐字执行 L120 封闭集将标记 P5 全部 10 模块，与冻结基线自相矛盾；(4) §3.2 L304“同 2 的校验”未钉顶层键违例与内层校验交互——W2 实现 = 期望键缺失 → LLMSIM_SCHEMA（refs=[键名, missing]）+ 多余键 → LLMSIM_UNKNOWN_KEY 每键 1 条，两族可同时出 + 该文件即停（不做内层校验），模块 docstring 已披露，保守确定选择（R4）；(5) ERR-P5-8（L964）影响面句“十项计数全部不变”括号枚举仅 9 项（缺“10 模块”）——勘误文本内部标签-枚举不一致（各站点实际计数互相一致：偏差链 5→8→10 自洽、§8.4 十行与 L919 吻合）；(6) W2 测试 docstring A 编号（A2/A3/A4/A5 = Leader 预裁定标签）与 §6.3 对抗表 A1-A13 编号冲突（SOT A 编号唯一归属对抗表）——代码侧修正（测试 docstring 改号，零行为变化）。→ 裁定与处置（Leader 2026-08-31 裁定；Fixer M 执行）：D1 L303 按 loc 序 → 按 pydantic errors() 产出序（即模型字段声明序；ERR-P5-13 钉死）；D2 L719 #20 公式 → `canonical_yaml(ir)` / `canonical_yaml(ir2)`（签名一致；两合取保留：数据级恒等 ir2 == ir + 双 dump 字节稳定）；D3 L120 白名单补 `__future__`；D4 L304 同 2 的校验 补顶层键违例交互注（两族可同时出 + 该文件即停，W2 实现口径，ERR-P5-13 钉死）；D5 L964 ERR-P5-8 枚举补“10 模块”（Leader 授权勘误行 in-place 修正——ERR-P5-11 算术修正先例，记入本勘误影响面）；D6 代码侧：test_project_ir.py / conftest.py docstring A{n} → W2-A{n}（7 处）+ 新增必需节缺失专测（game.yaml 缺 scenario → 恰好 1 条 LLMSIM_SCHEMA refs=("scenario","missing") path="game.yaml" + 节文件诊断继续聚合——钉死“诊断集”全聚合口径）+ test_canonical_yaml 补 #20 字面第二合取直断（canonical_yaml(ir2) == first）。10 INFO（R4-INFO-2 与 R1-INFO-2 同项）：6 不处置（R1-INFO-1 私有名 _ContractModel import = W1 先例 / R1-INFO-2 步 1 非 dict 双保险防御分支正常流不可达 / R3-INFO-2 load_project 五类病态探针不 raise 实测通过 / R4-INFO-2 同 R1-INFO-2 / R4-INFO-3 flatten (casefold, id) 全序超最低要求已披露 / R4-INFO-4 足迹透明审计注记）+ 4 吸收（R2-INFO-1 必需节缺失专测 → D6 / R2-INFO-2 再 dump 直断 → D6 / R3-INFO-1 必需节缺失诊断形态 = SOT 静默实现选择 → D6 新测试 + 模块 docstring 钉死 / R4-INFO-1 步 2/步 3 顶层键处置不对称 → D4 文档钉死）。→ 影响面：10 模块 / 20 断言 / 116 导出（25+6+6+11+43+8+4+3+3+7）/ 39 白名单 / 18 码 / 23 节点种类 / 66 parity / 13 对抗项 / 10 偏差 / 17 决策**全部不变**；无新文件 / 新导出 / 新码 / 新偏差 / 新决策；文档行数 993 → 994（+1 = 本勘误行）；ERR-P5-1..7、9..12 文本未动，ERR-P5-8 影响面枚举 1 处 in-place 修正（Leader 授权，本项）；W2 代码侧改动仅 test_project_ir.py 与 conftest.py（docstring 改号 + 1 新测试 + 1 直断），project_ir.py / loader.py 行为面零变化（套件 2434 → 2435）。W2 审查闭合（R1 一轮达标）→ 进入 W3 与 W5（module_graph ∥ plugins，W2 后可并行；W4 最重单独串行在 W2 后）。
**ERR-P5-14**（2026-08-31）：症状 → P5 W3（content/module_graph + test_module_graph）∥ W5（plugins manifest/api/registry + plugins 测试包）代码盲审，W3 三轮 + W5 三轮（每轮 4 盲审；终审 W3-R3 + W5-R3 8/8 通过、0 BLOCK / 0 SUPPLEMENT，gate 闭合）。各轮 BLOCK/SUPPLEMENT/DOC 裁定如下——W3-R1（1 BLOCK / 1 SUPPLEMENT / 6 DOC）：BLOCK = 版本比较 int() 换算，域内（文法 \d+(\.\d+)* 不限位数）≥4301 位分量触发 CPython 3.12 int() ValueError（get_int_max_str_digits=4300），违反 never-raise（模块头声明 + SOT L305/L347/L426）；SUPPLEMENT = check_module_versions 缺 SOT §3.4 L346 明确分派的解析错误复扫（非法 requires 条目仅报 REQUIRES_MISSING，不产 LLMSIM_MODULE_VERSION）；6 DOC = §3.4 L342（ModuleEdge 三字段面无 version_range）vs L350（「对每条带 version_range 的边」）字面不自洽、「去空白」trim 歧义、L742 测试行环诊断生产面归属（归 W6 validate_project）、模块头 L17「环提取 → 诊断列表」漂移、D-P5-06 L608 示例链「2.1 < 2.1.0」与其自身补 0 规则自相矛盾、测试文件两套编号分歧。W5-R1（1 SUPPLEMENT / 3 DOC）：SUPPLEMENT = EP 名/distribution 版本违 manifest pattern → discover_entry_point_plugins 之 PluginManifest 构造抛未捕获 pydantic ValidationError、发现中途中断零诊断（SOT L502 仅钉合法支、18 码闭集无专属通道）；3 DOC = §3.8 L462 导入面漏 pydantic、L502「origin=」诊断属性在 Diagnostic 5 字段契约内不可逐字表达（应落 refs）、EP 名 pattern 违例行为未定义。W3-R2（4/4 通过、3 DOC）：W3-A1..A9 预裁定引用 17 处无仓库可核查裁定文本；边面约束恢复措辞仍用旧「SOT 沉默处最保守读法」表述（L350 已原位钉死）。W5-R2（4/4 通过、4 SUPPLEMENT / 1 DOC，4 SUPPLEMENT 同根）：EP.value 空串 / path_label 空串 vs Diagnostic.path min_length=1（SOT §3.1 L167）→ EntryPointSpec.from_string(空串) 与 registry path=EP.value=空串 构造点抛未捕获 ValidationError、违反 never-raise；DOC = registry.py 模块 docstring 导入面枚举漏 pydantic。终审 W3-R3 + W5-R3（8/8 通过、0 BLOCK / 0 SUPPLEMENT、4 DOC）：module_graph.py L407-413 边面 docstring 描述边面不可达的裸 V 精确/文法外分支（_version_satisfied 共用语义照搬）；L301 find_cycles 外层序过引「§3.4 L348 / D-P5-15 L659-664」（SOT 仅钉逐 SCC 内层 casefold 序）；test_registry.py L23 残留「F-2」标签（SOT §9 F-2 属 ERR-P5-2）。→ 裁定与修正（Leader 2026-08-31 裁定；10 处 SOT 原位修正 + Fixer N/O/Q/P + Leader 终审后直接 docstring 修正 3 处 + 残留清理 12 处）：SOT 原位修正 10 处（9 行、L502 两处，均带 ERR-P5-14 锚点）：L350 = check_module_versions 三段条款（解析复扫面：节点 casefold 序 × requires/optional 声明序、解析错误 → LLMSIM_MODULE_VERSION path=node.id refs=(entry,)、(path,refs) 去重首次保留；边面：约束自 source 节点对应 kind 声明串首个解析至本边 target 元素恢复、归一形；节点面：空串 = 任意 / >=V / 裸 V 精确）与边面约束恢复口径；L462 = §3.8 导入面 += pydantic（Field/ValidationError）；L493 = §3.10 导入面 += pydantic（仅 ValidationError）；L502 = 逐 EP 首条款 += EP.value 空串 → LLMSIM_PLUGIN_ENTRY_INVALID（path=EP.name，refs=[EP.value]）+ 字段 pattern 违例 → 逐条 LLMSIM_SCHEMA（path=distribution 名或 EP.name，refs=(loc 点分串,type)）并跳过不注册（try/except 先于同名覆盖）；L479 = parse_plugin_manifest path_label = 非空文件键（调用方契约、空 label 域外）；L488 = EntryPointSpec.from_string s 空串 = 域外（registry 路在调用前将空值短路为 ENTRY_INVALID）；L506 = UNRESOLVED 条款 severity=warning refs=[descriptor.source 值]；L608 = D-P5-06 版本比较钉为点分数字串补 0 比较（「2 < 2.1，补 0 后 2.1 = 2.1.0」，零 int() 调用）；L742 = §6.1 test_module_graph 行（topological_order=[] + find_cycles 恰好 1 SCC [a,b,c]；诊断生产面 = W6 validate_project §3.6 每环一条 LLMSIM_MODULE_CYCLE path=min(node) refs=节点序）。代码修复：Fixer N（6 项 applied：F-1 纯数字串版本比较零 int()（组件对齐补 0 + 去前导 0 + 先长度后字典序，与数值补 0 比较等价）+ F-1T 5000 位/前导零断言 + F-3 三段式 check_module_versions 含解析复扫面等）；Fixer O（5 项 applied：EP 路 PluginManifest 构造 try/except ValidationError → 逐条 LLMSIM_SCHEMA 并跳过 EP、never-raise 恢复等）；Fixer Q（2 项 applied：D-1 边面约束恢复改 L350 钉死口径、D-2 全部 17 处 W3-A 引用改写为 SOT 条款锚点、全仓 W3-A=0）；Fixer P（5 项 applied：EP 空值短路守卫（from_string 之前）+ 4 处 docstring 域披露/导入面修正 + 1 新测试）。终审后 Leader 直接 docstring 修正 3 处（doc-level、零行为变化、按 doc-level 修正免再审政策）：module_graph.py L301 外层序归属改写（实现侧确定性钉死：SOT 仅钉逐 SCC 内层序）、L407-413 边面段标注共用比较面语义（裸 V/文法外形为节点/manifest engine_version 字段专属、SOT L350）、test_registry.py L23「F-2」→ ERR-P5-14。残留 wave 裁定引用清理（Leader 直接 12 处、doc-level）：module_graph.py L268 W2-A3 → project_ir.flatten_entities 交叉引用；api.py/registry.py/test_manifest.py/test_registry.py/conftest.py 之 W5-A4/A6/A10/A11/A2 → 去标签或换 SOT 锚点；已闭合 W2 文件引用（test_project_ir.py W2-A2/A3/A4、content/conftest.py W2-A5）保持原样（已闭合 wave 观察项、不在 P5 审查面）。Leader 简报错误记录：W3-A5 预裁定作废（SOT 原文优先、开发者正确上抛）；reviewer 简报行号锚点漂移（已建发出前重核制度）。→ 影响面：20 断言 / 116 导出 / 39 白名单 / 18 码 / 23 节点种类 / 66 parity / 13 对抗项 / 10 偏差 / 17 决策 / 10 模块全部不变（原位修正均为既有条款之澄清、无新导出/码/文件、无 gate 面变化）；ERR-P5-1..13 文本未动；全量套件 2435（W2 闭合）→ 2466（W3/W5 交付 + N 修复）→ 2469（O 修复 +3 用例）→ 2470（P 修复 +1 用例、Q 同期全量一致），全程 0 failed；ruff clean；K8 12 名自扫 0 命中；W3（module_graph 11 导出 + test_module_graph 12 用例）与 W5（plugins 三模块 13 导出 + 23 用例）终审轮 8/8 通过闭合，W3+W5 关闭，下一波 W4（rule_module 单独串行）。
**ERR-P5-15**（2026-08-31）：症状 → P5 W4（rule_module 43 导出 + test_rule_dsl_parity 66 例 + test_rule_module 39 例）三轮盲审：W4-R1（3/4 报告落盘，R1 席位执行失败 = 1 执行失败，R2 4 名新评审员全轮接替裁定；4 SUPPLEMENT + 4 DOC + 8 INFO，0 BLOCK）：(S1) And/Or 短路求值违背 v1 parse+eval 融合急进右操作数（v1 condition_eval.py:100-112 无条件求值右操作数；v2 用 Python and/or 短路；4 个确定性反例——右操作数抛 ConditionEvalError（未定义变量/除零）而 v2 返回了值——违反 SOT §3.5 L417 通用等价条款 + L391）〔实质〕；(S2) parity 例①（condition 求值失败 warn+skip）缺 caplog 断言（SOT §6.2 L763 ① 明示 caplog 断言 + 规则跳过；映射表 row 71 误引属例② regex 失败的「无 caplog 断言」从句）；(S3) test_rule_module 缺 object_id-in-text 第三序用例；(S4) evaluate_condition(rng=None) + rand 族行为 SOT 沉默；4 DOC（悬挂预裁定标签族：Leader A4/A1、裸 A3 共 5 处，与 §6.3 对抗 A 编号冲突，ERR-P5-13 (6) 先例）；8 INFO → Fixer R：F-1 实质（_eval_value And/Or 急进求值、左异常先行，恢复 v1 :100-112）+ F-1D×3 docstring（模块 L25 + AndNode L343 + OrNode L351 急进口径）+ F-1T/F-2T/F-3T（急进钉死 + object_id-in-text 第三序 + caplog 断言补全）+ F-4D×4 标签清理（改写为 SOT/v1 锚）；套件 2571→2573。W4-R2（4/4 落盘；1 SUPPLEMENT + 2 DOC + 12 INFO）：(S1) §6.1 L744 custom 规则流四项缺「match+condition 双命中」与「custom > builtin 优先级」用例（实现正确、测试缺失；parity 文件 66 例固定无新增无删减、不可承载）〔补充〕；DOC = requires_roll 表达式形（实现 = v1 rules.py:118 逐字 rule.feasibility == "uncertain" vs SOT L406 or-guard 形，对全部 RuleSpec.feasibility 取值 value-equivalent）+ 裸 A3 标签；12 INFO → Fixer S：F-1T 双命中矩阵 + F-2T 优先级用例 + F-3D 标签；套件 2573→2575。W4-R3（终审轮 3/4 通过 + 1/4 补充内容；1 SUPPLEMENT + 3 DOC + 6 INFO）：(S1) W4R3-4-01 —— parse_dsl 空 path_label vs Diagnostic.path min_length=1（schemas.py L538）：except DslEvalError 分支内构造 Diagnostic(path="") 触发 pydantic ValidationError 自永不抛逃逸（响亮 fail-fast；域内零影响——66 parity + 39 v2 用例 + loader 接线均用非空 label，评审员独立探针复现）；SOT §3.1 L167 钉 Diagnostic.path 非空但未钉 parse_dsl path_label 非空调用方契约（与 W5-R2 4 SUPPLEMENT 同根，ERR-P5-14 已对 parse_plugin_manifest / EntryPointSpec.from_string 于 L479/L488 原位钉死，parse_dsl 未被覆盖）；DOC×3 = parity L40 映射表注记「未命中分支 outcome 不求值」与 SOT L391 / ERR-P5-6 G-8 急进语义相悖 ×2（两席独立发现：v1 parse_if（L78-85）与 v2 _eval_if_chain 均先求值未命中分支 outcome 后丢弃，真正被跳过者仅为命中后其余分支，本例可观测 (allowed, None) 不受影响）+ MulNode/DivNode docstring 锚 v1 :209-217 vs SOT L381 :212-218 引用漂移（v1 实际布局核验：def parse_mul_div@L209、除零检查@L216-217、函数体止@L220，SOT 锚正确）；6 INFO（v1 id 直查无 dict 守卫 vs v2 isinstance-dict 过滤〔test_rule_module L464-468 已文档化〕、rule_module 无 K7 JSON 序列化面、_ContractModel 包内私有导入先例族等）→ W4-R4（终审轮 4/4 通过，0 BLOCK / 0 SUPPLEMENT / 0 执行失败；2 DOC + 6 INFO）→ Leader 轮后 DOC 修正 ×2（免费、零代码）：(D1) 映射表 row 44（tests/test_rules.py:44）转录注记（100<120）算术错误——v1 _test_state strength=0.4 × STRENGTH_TO_KG_FACTOR(50.0, rules.py:9) = 20kg < banquet_table 120kg，更正为（20<120）；(D2) docstring「v1 @ bb12460」（rule_module L23 + parity L4）vs SOT L13/L355 冻结源钉 f0a1052——v1 四文件在 f0a1052 均无后续修改（git 核验），统一为「v1 @ f0a1052」；6 INFO（文件集 mtime 观察、永真断言 L528、dict 守卫观察等）→ 裁定与修正（Leader 2026-08-31；SOT 原位修正 1 处 + docstring 修正 2 处 + 轮后 R4 DOC 修正 2 处，零代码改动）：(1) W4R3-4-01 = 调用方契约先例延伸至 parse_dsl，SOT §3.5 L390 原位钉死（「path_label = 非空标签（调用方契约；空 label = 域外响亮 fail-fast）」），零代码改动、行为不变（pydantic ValidationError 响亮 fail-fast）；(2) parity L40 注记改为「未命中分支 outcome 求值后丢弃，命中后其余分支不求值」；(3) MulNode/DivNode docstring 锚对齐 SOT L381（:212-218，除零 :216-217 口径）。Leader 裁定累计 3 项：(a) rng=None fail-fast 批准（SOT L391 钉 rng 必填非 None，L404/L411 钉 check_action_feasibility 默认面，保守安全，SOT 沉默面 = 域外输入）；(b) requires_roll 保留 v1 逐字形（value-equivalent，SOT L406 or-guard 注明）；(c) custom 规则「condition 未命中」≠ 规则跳过——仅 ConditionEvalError（求值失败）warn+skip（v1 rules.py:94-119 等价探针：condition 正常求值 → 结果无条件返回（即使 allowed）、matched_rule=custom:<id>）。开发者 8 偏差登记（D-P5-DEV-3/6/7/9 族 + 裸 uncertain 0.5 + rng None + TargetRef.source + 括号分组/排序/re.error 处理，详见 dev report）。影响面：SOT 995→996 行（本条 1 行 + L390 原位修正 1 行）；十计数全部不变（20/116/39/18/23/66/13/10/17/10）；套件 2470→2571→2573→2575（R4 DOC 修正 = docstring only，套件不变），ruff clean，K8 12 名自扫 0 命中；W4 实质轮消耗 1/3（R1 S1 急进修正；R4 2 DOC 为轮后免费修正、不占实质预算）；W4 关闭 = rule_module 43 导出（与 §8.2 台账逐名逐序一致）+ test_rule_dsl_parity 66 例（v1 41+25 逐字 1:1 转录、seeded 精确值、例① caplog 断言）+ test_rule_module 39 例（v2 面全族：23 种类 / 两阶段 / DslRng 注入 / resolve_variable / resolve_target 三序 / custom 规则流四项 / and-or 急进右操作数异常传播 / BUILTIN_RULE_IDS 逐字）。
**ERR-P5-16**（2026-08-31）：症状 → P5 W6（validator 8 导出 + cli 4 导出 + 10 fixtures #30-39 + 5 测试文件 89 例，T08/T09/T10；开发者 1 次执行成功 0 失败）开发前置与评审处置：(a) 3 处开发前原位修正（Leader 授权，记入本勘误）——§3.6 L427 与 §6.3 A1 行 L773：check_duplicate_ids refs = [id, 首次出现文件, 重复出现文件] → [id, 首次出现索引, 重复出现索引]（池 tuple 内 0-based 位置索引、字符串化；tuple 序 = build_ir sorted 路径合并序，确定性）——根因 = W1 schemas 冻结核验确认 IR 全模型零文件溯源字段（check_duplicate_ids(ir) 签名仅取 ir，原措辞输出不可由冻结输入产生，与 W4R3-4-01 同类）；§3.6 L428：「location 型引用（connection / player position）跳过」→「location 型引用（connection 值）跳过」——根因 = PlayerSpec/CharacterSpec.position 为 PositionSpec x/y/z 坐标非 location 引用；(b) W6-R1 盲审（4/4 通过，0 BLOCK / 0 SUPPLEMENT / 3 DOC / 6 INFO，0 执行失败；套件 2669/0，ruff clean，K8 12 名自扫 29 文件 0 命中）3 条 DOC 轮后免费修正（不占实质预算，W3/W4/W5 先例）：D1 cli.py L86/L117 docstring「§3.7 L452」→「§3.7 L454」（L452 = __all__ 行、L454 = main 规格行，引用漂移 2 行，零行为变化）；D2 §3.7 L450 stdout 纪律括注原位澄清——原文「（stderr 不限制，P5 实现零 stderr 输出）」与 L454 usage 错路径（parser.error 覆写 → stderr → return 2）矛盾且「任何模式」字面覆盖 --help 路径 → 改为「（唯一例外：usage 错路径按 main 规格写 stderr 并返回 2；--help 路径 stdout 输出帮助文本并返回 0——D-02 规格沉默面，实现取 argparse 标准面；除此之外 P5 实现零 stderr 输出）」；D3 §3.12 白名单 #29 行措辞修订——「纯追加 P5 块」→「纯追加 P5 块 + T06 单一受控偏离（仅 test_p5_integration.py 网络/进程 IO 类别豁免，§6.5 要求，provider/v1 零容忍保持）」；(c) 6 条 INFO 备案（零行动）：min(cycle)≡casefold 首元（_MODULE_ID_PATTERN 仅小写开头，码点序恒等，探针实测）；gate P4 探针分支取 openai 名词依赖行（test_validator 为探针表头指认的实现依据，SOT P4 行 provider 名词口径在同文件矩阵覆盖）；#16 断言于 load 级（loader 步 3 即生产点，raw=None 后 build_ir 不可达）；fixture 面 3 条备案；D-03/D-06 标签记录口径不一致（惯例层面，不采纳——dev 报告 deviations 已全量记录，零行为影响）；(d) Leader 裁定（累计 3）：R1 T06 豁免（D-05 解决）——SOT §6.5 字面 subprocess 冒烟 > T06 P3 期全树机械谓词，豁免最小化（单文件、仅 网络/进程 IO 类别、provider/v1 零容忍保持），§3.11 纯追加纪律受控偏离已记入代码注释与本勘误 D3；R2 gate 六 = -m 降级面（本环境 venv 无 pip、console script 未装配，SOT §6.5 降级条款，G5 报告披露）；R3 测试侧 importlib.import_module 许可（test_assert_17 以固定 10 路径元组枚举 __all__ 断言 116 导出唯一性——SOT 机械面仅覆盖 10 src 模块，测试侧扫描面 12 名/provider+网络库根/v1/T06 全谓词均不禁止 importlib，零动态加载风险，零改动）；(e) 开发者偏差 D-01..D-07 全部规格沉默最保守读法或 SOT 测试列字面落点（D-01 fixture 镜像省略 v1 无对应键 / D-02 --help rc 0 + stdout / D-03 usage stderr 单行前缀 / D-04 唯一性断言内置 test_assert_17 / D-05 T06（Leader 已裁定）/ D-06 A7 落点 test_validator / D-07 gate 降级），W6-R1 逐条复核通过。→ 影响面：十计数全部不变（20 断言 / 116 导出 = 25+6+6+11+43+8+4+3+3+7 / 39 白名单 / 18 诊断码 / 23 DSL 种类 / 66 parity / 13 对抗 / 10 偏差 / 17 决策 / 10 模块——W6-R1 独立重算逐一对账）；套件 2575 → 2669（W6 dev +89：test_validator 48 / test_cli 9 / gate 20 / adversarial 7 / integration 5；Leader P5 块 +5 TestP5Boundary 方法；T06 fail→pass），0 failed，ruff clean，K8 自扫 0 命中；W6 实质轮消耗 0/3（R1 一轮 4/4 通过，3 DOC 轮后免费修正）；W6 关闭 = validator 8 导出 + cli 4 导出（与 §8.2 L898/L899 台账逐名逐序一致）+ 10 fixtures（#30-39）+ 5 测试文件 89 例 + Leader 2 hunks（pyproject.toml [project.scripts] 3 行块；test_import_boundary.py P5 块 = P5_SUBMODULES 10 茎 + P5_TEST_FILES 15 项 + TestP5Boundary 5 方法 + T06 受控偏离）。
