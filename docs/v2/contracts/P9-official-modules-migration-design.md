...
- **D7 K8 词表闭集**：P9 src 字符串字面量 12 名零命中（openai / anthropic /
  ...
  api_key / base_url；P4 §3.4 黑名单 L187–200、12 名闭集口径 L199）+ engine_v2 全树 import 零
...
注：Spec §44 modules/ 树（L2145–2154）仅列 13 模块目录中 9 个（缺 actions/dialogue/
dynamics/narration，与 ERR-P9-03 占位 docstring 同一 4 名集）；13 名闭集以 Spec §40
（`OFFICIAL_MODULE_IDS`，§3.1.2 / A18）为唯一权威；§44 树按 Spec 内部不一致对待，
P9 不采用（ERR-P9-03 先例）。
...
| tactical | `("llmsim-standard-actions", "llmsim-standard-space")` | 战术模式限动作集 + 战术移动（Spec §41（L1970–1985）示例另含 standard.attributes；示例非规范闭集，SOT 按实际依赖面声明，§2.4） |
...
| `evaluate_lock_condition` | `(fields, actor_id: str, name: str, condition_dsl: str, rng: DslRng, tick: int) -> bool` | `condition_dsl` = P5 完整 `if(...)` if-chain 串（parse_dsl :812 根产生式；裸表达式 = 结构错误）；经 P5 `parse_dsl` + `evaluate_condition`（`DslRng` 注入）；结构错误 → 抛 `DslEvalError`（诊断面外化，不吞）；语义错误 = `DslEvalError` 透传；返回 = `outcome.feasibility is Feasibility.ALLOWED` |
...
| `derive_attributes` | `(fields: Mapping[str, AttributeField], actor_id: str, spec: Mapping[str, str], rng: DslRng) -> Mapping[str, AttributeField]` | 派生属性：`spec = {派生名: 裸 DSL 表达式}`，模块内部合成为 if-chain `if(<expr>, allowed; blocked)`（唯一包裹点；spec 对外保持裸表达式形），经 P5 `parse_dsl` + `evaluate_condition`（`DslRng` 注入）；派生 `AttributeField.value` = allowed 时 1.0 / 非 allowed 时 0.0（truthy 化；v1 数值算术派生 = DEV-P9-06）；派生结果不反向写源字段 |
...
| 名 | 形 | 语义 |
|---|---|---|
| `ItemState` | frozen dataclass：`id: str`、`name: str`、`description: str = ""`、`object_type: str = ""`、`position: Mapping[str, int] \| None = None`、`state: str \| None = None`、`properties: Mapping[str, object]`（缺省 `dataclasses.field(default_factory=dict)`；ERR-P9-08：原序列 position/properties 无默认值为 dataclass 语义拒绝，补默认值，字段名/型/序零改） | v1 object dict → v2 `ObjectSpec`（schemas.py:187）形状对齐；`state` = 扁平 str（§3.15.2 M-4 折叠规范）；`properties` 开放（D-P5-05 豁免） |
...
character yaml 形状（CharacterSpec:250 冻结面已承接）。Spec §12.3 CharacterState
（L852–856）声明 emotion/goals，但 P5 冻结面 CharacterSpec（content/schemas.py:250）
无此 2 字段 → P9 不建模（非 P9 面；如需建模，后移后续阶段承接）。
...
1. **A12 hex space**：`hex_adjacency` 边表 = 期望对称边集（3×3 = 16 无向边 / 32 有向，
...
| A12 | tactical 样例：3×3 hex 邻接边表 = 16 无向边 / 32 有向（对称去重）+ 对角 hex `distance_between` = 2；`GridSpace(3,3)` 曼哈顿对照值钉 | `test_g9_tactical.py::t1_hex_space` |
...
| §23 Scheduler/Time Contract（L1278） | §2.1（scheduler.py:374/468 + clock.py:77/117 冻结消费面）+ §3.16.2（A7/A8） |
| §24 Space Contract（L1341） | §3.11（hex/space 模块规范）+ §2.1（space.py:256/350 冻结消费面）+ A12 |
| §27 GameSystem/Python 扩展（L1500）/ §28 Plugin System（L1516）/ §29 `llmsim add`（L1547） | P9 非责任：System/Plugin 注册机制 = P5 冻结面（plugins 包，D5）；§27「MUST NOT 直接写」= K2（§8.1）；llmsim add CLI = presentation 面（§0.4 首行，P10/P11） |
...
### 8.4 偏差登记（DEV-P9-01..06；W0 时点全部已有处置）
...
| DEV-P9-06 | v1 派生属性 = 数值算术（`_ComputeParser` if 表达式产出数字，attributes.py:446）；P5 冻结 DSL 求值原语仅 `evaluate_condition`（产出 feasibility，无数值结果面） | 有意收窄（43.2 型）：v2 派生属性 = truthy 化（allowed→1.0 / 非 allowed→0.0，§3.2 `derive_attributes` 行钉）；数值派生能力移除，仓内 fixture 零消费（W5 迁移面 M 规则不涉及派生属性）；登记 §9 ERR-P9-07 |
