...
def apply_checkpoint(runtime: RuntimeState, instance_id: ActionInstanceId, *,
    ...
    + 入队下一 checkpoint（now + checkpoint_interval，interval 非 None 时——由 Scheduler 透传
    time_policy.checkpoint_interval_ticks（D-P3-13）；None → 不入队下一 checkpoint）。
    纯 RuntimeState 簿记：
...
class Scheduler:                                     # 新增（门面；**不是**真相——K7）
    def __init__(self, registry: ActionRegistry, *,
                 ...
                 origin: Provenance,                             # 必填（E-P3-40/F5-01）：run()-级事务 origin
                                                                 # （provenance.py:71-72 双必填构造）；Gate fixture =
                                                                 # Provenance(producer_id=origin_scenario,
...
### E-P3-40（实现期 Wave C，Leader 裁定两项）`apply_checkpoint` 间隔通道缺失 + `Scheduler.__init__` 漏 run()-级 origin 参数
- **内容**：① §3.6 `apply_checkpoint` docstring 要求"入队下一 checkpoint（now + TimePolicy 间隔，若策略开启）"，但钉死签名 `(runtime, instance_id, *, at_tick, current_revision)` 无间隔通道——`TimePolicy.checkpoint_interval_ticks` 属 scheduler.py（§3.2 依赖图禁止 action_lifecycle 反向 import scheduler），P1 冻结 `ActiveAction`（actions.py:207-244）无间隔字段；实现期推演证明 S4（t=10）与 A2（t=20）两刻无统一公式可同时推导 interval（详 T04a 报告）。现签名增关键字 `checkpoint_interval: int | None = None`（Scheduler 透传 `time_policy.checkpoint_interval_ticks`；None → 不入队下一 checkpoint），与同文件 `start_action` 的 `checkpoint_interval` 关键字同款先例。② E-P3-34/F5-01 钉死 run()-级 origin（"随装配显式传入"）但 §3.8 `Scheduler.__init__` 签名草图漏列该参数——现增必填 `origin: Provenance`（provenance.py:71-72 双必填构造），Scheduler 内部所有 `run()` 提交统一用它；§5.1 调度器构造行同步补 `origin=Provenance(producer_id=origin_scenario, origin=OriginKind.SCENARIO)`。
