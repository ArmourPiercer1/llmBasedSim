...
3. .venv/bin/python -m pytest -q --tb=no
   # 全量：期望 3054（§8.3；= 3048 + 6 边界纯追加，ERR-P8-05）
...
边界锚纯追加（§3.10.3 TestP8Boundary） = 6
─────────────────────────────────────────────
gate1_p8_face（§9 历史条目引用口径）   = 3048
gate1_expected（门禁 ③ 全量套件）      = 3054
```

恒等式约束：`§6.1 函数总数 == 123 == W 波次表 Σ（26+24+25+26+22）==
A↔函数映射非 A 部分（101）+ A（22）`。实现波次若新增/删减测试函数，
**必须**同步更新本恒等式与报告 `counts`（W0 纪律，P7 §8.3 同族）。
§3.10.3 的 6 方法为锚文件（`core/test_import_boundary.py`）纯追加、
不计入 §6.1 函数数（123 恒等式不变）；门禁 ③ 全量期望 = 3048（基线 +
§6.1；§9 历史条目引用口径）+ 6 = **3054**（ERR-P8-05 修正）。
...
- **ERR-P8-05**（W5 prep 触发；§8.3 / 门禁步 3 全量期望值修正 3048 → 3054）：
  触发文件:行：无代码触发（W5 派发前 SOT 自洽性审查）——§3.10.3（L921–930：
  TestP8Boundary 6 方法、锚文件纯追加）× §8.3（gate1_expected = 3048 =
  2925+123）× §3.10.4 步 3（全量期望 3048）内部不一致：6 方法不计入 §6.1
  函数数（123 恒等式不含之），而门禁 ③ 在 W5 收口（6 方法已落位）后运行，
  全量实测必为 3054 ≠ 3048；
  修正口径：§8.3 增「边界锚纯追加（6）」项 + gate1_p8_face（3048 = 2925+123，
  §9 历史条目引用口径保留）/ gate1_expected（门禁 ③ 全量 = 3054）两行；
  §3.10.4 步 3 注释同步（3048 → 3054）；恒等式约束段补 6 方法归属注；
  影响面：§6.1 123 恒等式不变（26+24+25+26+22）、白名单 25 文件不变、
