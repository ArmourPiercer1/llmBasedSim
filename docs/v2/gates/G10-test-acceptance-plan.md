...
> 启动可加 `--living`（活世界宿主）：每次动作时间 +3h（24h 循环）、
> 天气 / 位置 / mood 周期 → 图像背景色随之变化；缺省静态宿主 = 世界
> 内容不变、图像不变（by-design，触发面 = G10 报告 §9.3）。

> **首轮执行注记**：2026-09-03 首轮人工判定已执行（判定人 = 用户），
> 结论并入 `G10-gate-report.md` §9.1/§9.2/§9.3（G10-5 部分可读 /
> G10-6 未见错场 / G10-7 不可判定 → P11 承接）；后续轮次（可选，
> 建议加 `--living` 复跑）等待判定人排期。
