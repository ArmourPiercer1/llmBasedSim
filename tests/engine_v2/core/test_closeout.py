...
class TestCorePackageReexports:
    ...
    def test_package_all_no_private_no_duplicates(self) -> None:
        ...
        # 规模锚点：154 个唯一名称（13 个契约模块 __all__ 共 95 项，
        ...
        # AUTHORITY_REASON_CODES / AuthorityEvaluationResult /
        # ProducerConflictError / ProducerInfo / ProducerRegistry）
        # + reducer 37 项（P2-T01）+ validation 11 项（P2-T04）
        # = 154；conflicts/transaction_executor/cascade 三个占位骨架
        # 模块（D-P2-19）尚无公开导出，随各自任务包落地时本锚点同步）
        assert len(core_pkg.__all__) == 154
